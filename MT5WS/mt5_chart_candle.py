...
class CandleChart(tk.Canvas):
    def __init__(self, master, rates, info_labels, symbol_short, **kwargs):
...
def main():
    ...
    drag_gap = 10       # キャンドルチャートとドラッグエリアの間隔
    drag_width = 10      # ドラッグエリアの幅
    height = 300
    total_width = info_width + chart_width + drag_gap + drag_width
    ...
    # チャートの位置（情報エリアの右）
    chart = CandleChart(root, rates, info_labels=info_labels, symbol_short=symbol_short, width=chart_width, height=height, bg='white', highlightthickness=0)
    chart.place(x=info_width, y=0)

    # ドラッグエリア（右端）
    drag_area = tk.Frame(root, bg='gray', width=drag_width, height=height)
    drag_area.place(x=info_width + chart_width + drag_gap, y=0)
    ...
    root.bind("<Up>", increase_opacity)
    root.bind("<Down>", decrease_opacity)
